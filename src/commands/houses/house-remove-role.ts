...
import { hasAssignRolePermission, userRemoveRole } from './house-utils.js';

export default {
  data: {
    name: "house-remove-role",
    description: "Removes an arbitrary role from all members of a house (e.g. for winning the house cup)",
    options: [
      {
  ...
  async execute(interaction) {
    ...
    for (const user of usersInHouse) {
      userRemoveRole(guildId, user.discord_id, BigInt(role.id))
    }
